...
export const citasApi = createApi({
  ...
  endpoints: (builder) => ({
      ...
      transformResponse: (response: any[]) => {
        ...
        return response.map(c => ({
          ...
          medico: c.medico ? {
            ...
            calificacion: c.medico.calificacion ? parseFloat(c.medico.calificacion) : 0,
    ...
    getHorariosDisponibles: builder.query<HorarioDisponible[], { medicoId: number | string; fecha: string }>({
      query: ({ medicoId, fecha }) => `/citas/horarios-disponibles?medicoId=${medicoId}&fecha=${fecha}`,
      transformResponse: (response: any[]) => {
