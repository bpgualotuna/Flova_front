...
export default function TherapySelectionPage() {
  ...
  const handleSelectTerapia = (terapia: Terapia) => {
    ...
    // Clave versionada: si el schema de Terapia cambia en el futuro,
    // se incrementa la versión (v2, v3...) para ignorar datos viejos
    // y evitar crashes al parsear un formato obsoleto.
    sessionStorage.setItem("selectedTerapia:v1", JSON.stringify(terapia));
    // Navegar a selección de médico
    navigate("/seleccion-medico");
  };

  // Filtrar terapias por búsqueda
  const filteredTerapias = terapias.filter(
    (t) =>
